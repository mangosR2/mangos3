...
pAuraHandler AuraHandler[TOTAL_AURAS]=
{
    ...
    &Aura::HandleAuraModCategoryCooldown,                   //341 SPELL_AURA_MOD_SPELL_CATEGORY_COOLDOWN 2 spells in 4.3.4 implemented in Unit::AddSpellAndCategoryCooldowns
...
Unit* Aura::GetTriggerTarget() const
{
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        if (i != GetEffIndex())
        {
            ...
            {
                if (aur->GetModifier()->m_auraname == SPELL_AURA_DUMMY)
...
bool Aura::IsEffectStacking()
{
    ...
    // generic check
    if (spellProto->GetAttributesEx6() & (SPELL_ATTR_EX6_NO_STACK_DEBUFF_MAJOR | SPELL_ATTR_EX6_NO_STACK_BUFF))
    {
    ...
    switch(GetModifier()->m_auraname)
    {
        ...
        case SPELL_AURA_MOD_MELEE_HASTE:
            if (spellProto->GetSpellFamilyName() == SPELLFAMILY_GENERIC)
                return true;
            // Icy Talons
        ...
        case SPELL_AURA_MOD_STAT:                                                              // various stat buffs
            return (spellProto->GetSpellFamilyName() == SPELLFAMILY_GENERIC);
        ...
        case SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN:                                              // Ebon Plague (spell not implemented) / Earth and Moon
            if (spellProto->IsFitToFamily<SPELLFAMILY_WARRIOR, CF_WARRIOR_SUNDER_ARMOR>() ||   // Sunder Armor (only spell triggering this aura has the flag)
                ...
                (spellProto->GetSpellFamilyName() == SPELLFAMILY_DRUID && spellProto->GetSpellIconID() == 2991) || // Earth and Moon
        ...
        case SPELL_AURA_MOD_CRIT_PERCENT:                                                      // Rampage
            if (spellProto->GetSpellFamilyName() == SPELLFAMILY_WARRIOR && spellProto->GetSpellIconID() == 2006)
                return false;
            break;
        ...
        case SPELL_AURA_MOD_ATTACKER_SPELL_HIT_CHANCE:                  // Misery / Imp. Faerie Fire (must find triggered aura / spell not implemented?)
            if (spellProto->GetSpellFamilyName() == SPELLFAMILY_PRIEST &&
                spellProto->GetSpellIconID() == 2211)                        // Misery
            {
                return false;
            }
            break;
    ...
    if (GetSpellEffect()->Effect == SPELL_EFFECT_APPLY_AREA_AURA_RAID)
        return false;
    ...
    else                                                    // initial triggeredSpellInfo != NULL
    {
        ...
        switch(auraId)
        {
            ...
            case 6474:
            {
                ...
                {
                    if ((*itr)->GetSpellProto()->GetSpellIconID() == 2289 && (*itr)->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_SHAMAN)
                    {
...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
            ...
            case SPELLFAMILY_ROGUE:
            {
                ...
                if (GetId() == 52916)
                {
                    ...
                    for (Unit::AuraList::const_iterator i = procTriggerSpellAuras.begin(); i != procTriggerSpellAuras.end(); ++i)
                    {
                        ...
                        if (spellInfo->GetSpellEffect(EFFECT_INDEX_0)->EffectTriggerSpell == 52916)
                        {
                            // Get caster of aura
    ...
    else
    {
        ...
        // Living Bomb
        if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_MAGE && GetSpellProto()->GetSpellFamilyFlags().test<CF_MAGE_LIVING_BOMB>())
        {
    ...
    switch(GetSpellProto()->GetSpellFamilyName())
    {
        ...
        {
            // Haunt
            if (GetSpellProto()->GetSpellIconID() == 3172 && GetSpellProto()->GetSpellFamilyFlags().test<CF_WARLOCK_HAUNT>())
            {
    ...
    if (apply)
    {
        ...
        if (Real)
        {
            ...
            if (target->GetAurasByType(SPELL_AURA_MOD_STEALTH).size()<=2) // Vanish also triggering Stealth, but all ok, no double auras here possible
            {
                ...
                {
                    // Master of Subtlety
                    if ((*i)->GetSpellProto()->GetSpellIconID() == 2114 && GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_ROGUE)
                    {
...
void Aura::HandleModMechanicImmunity(bool apply, bool /*Real*/)
{
    ...
    // Demonic Circle
    if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_WARLOCK && GetSpellProto()->GetSpellIconID() == 3221)
    {
        if (target->GetTypeId() != TYPEID_PLAYER)
            return;
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch(GetSpellProto()->Id)
            {
                ...
                case 61474:                                   // Grip of Slad'ran (h)
                {
                    if (apply)
                    {
                        ...
                        if (GetHolder()->GetStackAmount() >= 5)
                        {
                            ...
                            target->CastSpell(target, (GetSpellProto()->Id == 55093) ? 55126 : 61476, true);
                        }
                    }
                    break;
                }
                ...
                case 63477:
                {
                    ...
                    if (!apply)
                        target->CastSpell(caster, (GetSpellProto()->Id == 62717) ? 62836 : 63536, true);
        ...
        case SPELLFAMILY_WARLOCK:
        {
            switch (GetSpellProto()->Id)
            {
                ...
                case 48018:
                {
                    ...
                    {
                        target->RemoveGameObject(GetSpellProto()->Id,true);
    ...
    if (GetId() == 91565)
    {
        if (Unit* caster = GetCaster())
        {
            ...
            for (Unit::AuraList::const_iterator itr = mDummyAuras.begin(); itr != mDummyAuras.end(); ++itr)
            {
                ...
                if (spell->GetSpellIconID() == 960 && spell->GetSpellFamilyName() == SPELLFAMILY_DRUID &&
...
void Aura::PeriodicTick()
{
    ...
    switch (m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
            ...
            {
                uint32 pdamageReductedArmor = pCaster->CalcArmorReducedDamage(target, damageInfo.damage, NULL);
                damageInfo.cleanDamage += damageInfo.damage - pdamageReductedArmor;
                damageInfo.damage = pdamageReductedArmor;
            }

            // Curse of Agony damage-per-tick calculation
            if (spellProto->GetSpellFamilyName() == SPELLFAMILY_WARLOCK && spellProto->GetSpellFamilyFlags().test<CF_WARLOCK_CURSE_OF_AGONY>() && spellProto->GetSpellIconID()==544)
            {
        ...
        case SPELL_AURA_OBS_MOD_HEALTH:
        {
            ...
            else
            {
                ...
                if (classOptions && classOptions->GetSpellFamilyName() == SPELLFAMILY_DRUID && spellProto->GetSpellIconID() == 2864)
                {
                ...
                else if (spellProto->Id == 73651)
                {
                    ...
                    damageInfo.damage = int32(healthPct * target->GetMaxHealth() / 100.0f);
                }
            }
...
//              case 50530: break;
            ...
            switch (spell->Id)
            {
                ...
                case 55342:
                {
                    if (GetAuraTicks() != 1)
                        return;
                    if (Unit* pCaster = GetCaster())
                        pCaster->CastSpell(pCaster, GetSpellEffect()->EffectTriggerSpell, true, NULL, this);
                }
                default:
                    break;
            }
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            else if (GetId() == 93426)
            {
                ...
                SpellAuraHolderPtr infusion = pet->GetSpellAuraHolder(91342);
...
void Aura::HandleAuraLinked(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        else if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_DEATHKNIGHT &&
            (GetSpellProto()->GetSpellIconID() == 264 || GetSpellProto()->GetSpellIconID() == 1933))
        {
...
void SpellAuraHolder::_AddSpellAuraHolder()
{
    ...
    if (m_target->GetVisibleAurasCount() < MAX_AURAS)
    {
        ...
        for (size_t i = 0; i < MAX_AURAS ; ++i)
        {
            if (i == visibleAuras.size() || !visibleAuras[i])
            {
                slot = i;
...
void SpellAuraHolder::_RemoveSpellAuraHolder()
{
    ...
    if (caster && caster->GetTypeId() == TYPEID_PLAYER)
    {
        if (GetSpellProto()->GetAttributes() & SPELL_ATTR_DISABLED_WHILE_ACTIVE)
...
void SpellAuraHolder::HandleSpellSpecificBoosts(bool apply)
{
    ...
    switch (GetSpellProto()->GetSpellFamilyName())
    {
        ...
        {
            // Bandages
            if (GetSpellProto()->GetMechanic() == MECHANIC_BANDAGE && !apply)
            {
                cast_at_remove = true;
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            if (m_spellProto->Id == 120 && apply)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    for (Unit::AuraList::const_iterator itr = mDummyAuras.begin(); itr != mDummyAuras.end(); ++itr)
                    {
                        if ((*itr)->GetSpellProto()->GetSpellIconID() == 35 && (*itr)->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_MAGE)
                        {
                            // Rank 1
                            if ((*itr)->GetId() == 11190)
            ...
            else if (m_spellProto->GetSpellFamilyFlags().test<CF_MAGE_FIREBALL>() && GetSpellProto()->GetSpellVisual() == 67)
            {
                // Glyph of Fireball
                if (Unit * caster = GetCaster())
            ...
            else if (m_spellProto->GetSpellFamilyFlags().test<CF_MAGE_FROSTBOLT>() && GetSpellProto()->GetSpellVisual() == 13)
            {
                // Glyph of Frostbolt
                if (Unit * caster = GetCaster())
...
void SpellAuraHolder::HandleSpellSpecificBoostsForward(bool apply)
{
    ...
    switch(GetSpellProto()->GetSpellFamilyName())
    {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (m_spellProto->GetSpellFamilyFlags().test<CF_WARLOCK_MISC_DEBUFFS>() && m_spellProto->GetSpellIconID() == 2209)
            {
                switch(GetId())
                {
                    case 32386:
...
bool SpellAuraHolder::HasMechanic(uint32 mechanic) const
{
    ...
    for (int32 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        if (GetAura(SpellEffectIndex(i)) && GetAura(SpellEffectIndex(i))->GetSpellEffect()->EffectMechanic == mechanic)
            return true;
    }
    return false;
}
...
uint32 Aura::CalculateCrowdControlBreakDamage()
{
    ...
    if (GetSpellProto()->HasAttribute(SPELL_ATTR_EX_BREAKABLE_BY_ANY_DAMAGE) &&
        ((GetSpellProto()->GetAuraInterruptFlags() & AURA_INTERRUPT_FLAG_DIRECT_DAMAGE) ||
...
void Aura::HandleInitializeImages(bool apply, bool real)
{
    if (!real)
        return;
    ...
    if (!caster || !target || !target->IsInWorld() || target->GetTypeId() != TYPEID_UNIT)
        return;
    ...
    else
    {
        ...
        if (!cinfo)
        {
            sLog.outErrorDb("Aura::HandleInitializeImages try set name to unknown creature %s", target->GetObjectGuid().GetString().c_str());
            return;
        }
