...
void Spell::FillTargetMap()
{
    ...
    for(int i = EFFECT_INDEX_0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        for (uint8 j = EFFECT_INDEX_0; j < i; ++j)
        {
            ...
            if (spellEffect1->Effect == SPELL_EFFECT_NONE)
                continue;

            // Check if same target, but handle i.e. AreaAuras different
            if (spellEffect->EffectImplicitTargetA == spellEffect1->EffectImplicitTargetA && 
                spellEffect->EffectImplicitTargetB == spellEffect1->EffectImplicitTargetB
        ...
        if (!FillCustomTargetMap(spellEffect, tmpUnitLists[i], effToIndex[i]) && effToIndex[i] == i)
        {
...
void Spell::DoAllEffectOnTarget(TargetInfo *target)
{
    ...
    else if (m_damage)
    {
        ...
        if (classOpt && classOpt->SpellFamilyName == SPELLFAMILY_WARLOCK && m_spellInfo->GetSpellIconID() == 3172 &&
            classOpt->GetSpellFamilyFlags().test<CF_WARLOCK_HAUNT>())
    ...
    for (uint8 effectNumber = 0; effectNumber < MAX_EFFECT_INDEX; ++effectNumber)
    {
        ...
        {
            SpellEffectEntry const* effectEntry = m_spellInfo->GetSpellEffect(SpellEffectIndex(effectNumber));
            if (!effectEntry)
                continue;

            // Get multiplier
            float multiplier = effectEntry->DmgMultiplier;
    ...
    switch (targetMode)
    {
        ...
        case TARGET_RANDOM_NEARBY_DEST:
        {
            ...
            if (spellEffect->Effect == SPELL_EFFECT_SUMMON || spellEffect->Effect == SPELL_EFFECT_SUMMON_OBJECT_WILD)
                unMaxTargets = spellEffect->CalculateSimpleValue();
        ...
        case TARGET_TABLE_X_Y_Z_COORDINATES:
        {
            ...
            if (st)
            {
                ...
                if (st->GetMapId() != m_caster->GetMapId() && spellEffect && spellEffect->Effect != SPELL_EFFECT_TELEPORT_UNITS && spellEffect->Effect != SPELL_EFFECT_BIND)
...
void Spell::cast(bool skipCheck)
{
    ...
    switch(m_spellInfo->GetSpellFamilyName())
    {
        case SPELLFAMILY_GENERIC:
        {
            ...
            else if (m_spellInfo->GetMechanic() == MECHANIC_MOUNT)
            {
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            if (m_caster->GetTypeId() == TYPEID_PLAYER && m_spellInfo->IsFitToFamilyMask(UI64LIT(0x1000000)) &&
                m_spellInfo->GetEffectApplyAuraNameByIndex(EFFECT_INDEX_0) == SPELL_AURA_MOD_CONFUSE && m_spellInfo->GetSpellIconID() == 82)
            {
...
void Spell::SendEffectLogExecute(SpellEffectEntry const* effect, ObjectGuid targetGuid, uint32 data1, uint32 data2, float data3)
{
    if (!effect)
        return;
...
void Spell::SendChannelUpdate(uint32 time)
{
    ...
    WorldPacket data(SMSG_CHANNEL_UPDATE, m_caster->GetPackGUID().size() + 4);
    data << m_caster->GetPackGUID();
    data << uint32(time);
    m_caster->SendMessageToSet(&data, true);
}
...
void Spell::SendChannelStart(uint32 duration)
{
    ...
    WorldPacket data(SMSG_CHANNEL_START, m_caster->GetPackGUID().size() + 4 + 4);
    data << m_caster->GetPackGUID();
    data << uint32(m_spellInfo->Id);
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_caster->isCharmedOwnedByPlayerOrPlayer())
    {
        ...
        if (m_caster->GetTypeId() == TYPEID_PLAYER && ((Player*)m_caster)->isMoving() && !m_caster->HasAffectedAura(SPELL_AURA_ALLOW_CAST_WHILE_MOVING, m_spellInfo))
        {
    ...
    if(Unit* target = m_targets.getUnitTarget())
    {
        ...
        if (non_caster_target)
        {
            ...
            if (m_caster->GetTypeId() == TYPEID_PLAYER && m_spellInfo->GetMechanic() == MECHANIC_DISARM)
            {
                if (target->GetTypeId() == TYPEID_PLAYER)
                {
        ...
        else if (m_caster == target)
        {
            ...
            // Fire Shield
            if (classOptions && classOptions->GetSpellFamilyName() == SPELLFAMILY_WARLOCK &&
            ...
            if (classOptions && classOptions->GetSpellFamilyName() == SPELLFAMILY_PALADIN &&
        ...
        switch (spellEffect->Effect)
        {
            ...
            case SPELL_EFFECT_DUMMY:
            {
                    ...
                    && (m_spellInfo->Id == 20577 || m_spellInfo->Id == 52749 || m_spellInfo->Id == 54044))
                {
                    ...
                    WorldObject* result = FindCorpseUsing<MaNGOS::CannibalizeObjectCheck>(m_spellInfo->GetTargetCreatureType());
...
bool Spell::FillCustomTargetMap(SpellEffectEntry const* effect, UnitList& targetUnitMap, uint8& effToIndex)
{
    float radius;
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 12355: // Impact
        {
            if (effect->GetIndex() != EFFECT_INDEX_1)
                return false;
            ...
            SpellAuraHolderPtr dot = SpellAuraHolderPtr(NULL);
        ...
        case 57143: // Life Burst (Wyrmrest Skytalon)
        {
            ...
            SetTargetMap(effect->GetIndex(), TARGET_ALL_FRIENDLY_UNITS_AROUND_CASTER, targetUnitMap);
            targetUnitMap.push_back(m_caster);
            break;
        }
        ...
        case 58912: // Deathstorm
        {
            ...
            SetTargetMap(effect->GetIndex(), TARGET_RANDOM_ENEMY_CHAIN_IN_AREA, targetUnitMap);
            break;
        }
