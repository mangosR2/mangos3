...
//                plr->ModifyArenaPoints(win_arena);
        ...
        else
        {
            ...
            if (IsRandom() || BattleGroundMgr::IsBGWeekend(GetTypeID()))
                UpdatePlayerScore(plr, SCORE_BONUS_HONOR, GetBonusHonorFromKill(loose_kills));
        }
        ...
        sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, this, plr, plr->GetBattleGroundQueueIndex(bgQueueTypeId), STATUS_IN_PROGRESS, TIME_TO_AUTOREMOVE, GetStartTime(), GetArenaType(), plr->GetBGTeam());
        plr->GetSession()->SendPacket(&data);
...
void BattleGround::RemovePlayerAtLeave(ObjectGuid guid, bool Transport, bool SendPacket)
{
    ...
    if (participant) // if the player was a match participant, remove auras, calc rating, update queue
    {
        ...
        if (plr)
        {
            ...
            {
                WorldPacket data;
                sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, this, plr, plr->GetBattleGroundQueueIndex(bgQueueTypeId), STATUS_NONE, 0, 0, ARENA_TYPE_NONE, TEAM_NONE);
                plr->GetSession()->SendPacket(&data);
            }
...
void BattleGround::PlayerAddedToBGCheckIfBGIsRunning(Player* plr)
{
    ...
    sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, this, plr, plr->GetBattleGroundQueueIndex(bgQueueTypeId), STATUS_IN_PROGRESS, GetEndTime(), GetStartTime(), GetArenaType(), plr->GetBGTeam());
    plr->GetSession()->SendPacket(&data);
}
