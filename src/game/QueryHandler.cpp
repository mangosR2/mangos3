...
void WorldSession::SendNameQueryOpcode(ObjectGuid guid)
{
    ...
    data << guid.WriteAsPacked();                           // player guid
    if (!nameData)
    {
    ...
    if (DeclinedName const* names = (p ? p->GetDeclinedNames() : NULL))
    {
    ...
    if (corpsemapid != _player->GetMapId())
    {
        ...
        if (MapEntry const* corpseMapEntry = sMapStore.LookupEntry(corpsemapid))
        {
            if (corpseMapEntry->IsDungeon() && corpseMapEntry->ghost_entrance_map >= 0)
            {
                ...
                if(TerrainInfoPtr entranceMap = sTerrainMgr.LoadTerrain(corpseMapEntry->ghost_entrance_map))
                {
