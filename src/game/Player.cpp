...
void TradeData::SetSpell(uint32 spell_id, Item* castItem /*= NULL*/)
{
    ...
    Update(true, true);                                     // send spell info to item owner
    Update(false, true);                                    // send spell info to caster self
}
...
void TradeData::Update(bool for_trader /*= true*/, bool spellOnly /*=false*/)
{
    if (for_trader)
        m_trader->GetSession()->SendUpdateTrade(true, spellOnly);      // player state for trader
    else
        m_player->GetSession()->SendUpdateTrade(false, spellOnly);     // player state for player
}
...
void Player::GiveLevel(uint32 level)
{
    ...
    for (uint32 i = 1; i < MAX_STORED_POWERS; ++i)
        data << uint32(0);
    // Stats loop (0-4)
...
void Player::ApplyEnchantment(Item *item, EnchantmentSlot slot, bool apply, bool apply_dur, bool ignore_condition, bool ignore_skill)
{
    ...
    if (slot > PRISMATIC_ENCHANTMENT_SLOT && slot < PROP_ENCHANTMENT_SLOT_0 || slot >= MAX_ENCHANTMENT_SLOT)
        return;
...
void Player::_LoadVoidStorage(QueryResult* result)
{
    ...
    do
    {
        ...
        if (creatorGuid && !sAccountMgr.GetPlayerAccountIdByGUID(creatorGuid))
        {
...
void Player::SendModifyCooldown(uint32 spell_id, int32 delta)
{
    ...
    data << uint32(spell_id);
    data << GetObjectGuid();
    data << int32(result > 0 ? delta : result -int32(cooldown) * IN_MILLISECONDS);
    GetSession()->SendPacket(&data);
}
...
void Player::HandleFall(MovementInfo const& movementInfo)
{
    ...
    if (z_diff >= 14.57f && !isDead() && !isGameMaster() &&
        ...
        !HasAuraType(SPELL_AURA_FLY) && !IsImmunedToDamage(SPELL_SCHOOL_MASK_NORMAL) && !hasFlyMountAura && (!GetTransportInfo() || GetTransportInfo()->GetTransportGuid().IsEmpty()))
    {
...
void Player::CompletedAchievement(uint32 uiAchievementID)
{
    ...
    {
        if (!GetAchievementMgr().HasAchievement(uiAchievementID))
            GetAchievementMgr().CompletedAchievement(achievement, this);
    }
    else
