...
uint32 Unit::DealDamage(DamageInfo* damageInfo)
{
    ...
    if (damageInfo->cleanDamage && damageInfo->damageType == DIRECT_DAMAGE && this != pVictim && GetTypeId() == TYPEID_PLAYER && (getPowerType() == POWER_RAGE))
    {
        ...
        switch (damageInfo->attackType)
        {
            case BASE_ATTACK:
            {
                weaponSpeedHitFactor = uint32(GetAttackTime(damageInfo->attackType) / 1000.0f * 6.5f);
                if (damageInfo->hitOutCome == MELEE_HIT_CRIT)
                    weaponSpeedHitFactor *= 2;

                ((Player*)this)->RewardRage(damageInfo->damage + damageInfo->GetAbsorb(), true, pVictim);

                break;
            }
            case OFF_ATTACK:
            {
                weaponSpeedHitFactor = uint32(GetAttackTime(damageInfo->attackType)/1000.0f * 3.25f);
                if (damageInfo->hitOutCome == MELEE_HIT_CRIT)
                    weaponSpeedHitFactor *= 2;
    ...
    if (!vSchoolAbsorb.empty())
    {
        for(AuraList::const_iterator i = vSchoolAbsorb.begin(); i != vSchoolAbsorb.end() && RemainingDamage > 0; ++i)
        {
            ...
            switch (spellProto->GetSpellFamilyName())
            {
                ...
                {
                    // Ancestral Resolve
                    if (spellProto->GetSpellIconID() == 3923)
                    {
...
int32 Unit::DealHeal(DamageInfo* healInfo, bool critical/* = false*/)
{
    ...
    if (unit->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        ((Player*)unit)->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HEAL_CASTED, healInfo->heal);
    }

    if (pVictim->GetTypeId() == TYPEID_PLAYER)
    {
        ((Player*)pVictim)->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_TOTAL_HEALING_RECEIVED, gain);
        ((Player*)pVictim)->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HEALING_RECEIVED, healInfo->heal);
    }
...
void Unit::SpellDamageBonusDone(DamageInfo* damageInfo, uint32 stack)
{
    ...
    for (AuraList::const_iterator i = mDamageDoneByMechanic.begin(); i != mDamageDoneByMechanic.end(); ++i)
        if (int32 misc = (*i)->GetModifier()->m_miscvalue)
            if (GetAllSpellMechanicMask(damageInfo->GetSpellProto()) & (1 << (misc - 1)))
                DoneTotalMod *= ((*i)->GetModifier()->m_amount + 100.0f) / 100.0f;
    ...
    switch (damageInfo->GetSpellProto()->GetSpellFamilyName())
    {
        ...
        {
            // Frostbold
            if (damageInfo->GetSpellProto()->Id == 116)
            {
                if (pVictim->isFrozen() || IsIgnoreUnitState(damageInfo->GetSpellProto(), IGNORE_UNIT_TARGET_NON_FROZEN))
                {
                    ...
                    {
                        if ((*itr)->GetSpellProto()->GetSpellIconID() == 976 && (*itr)->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_MAGE)
                        {
            ...
            // Ice Lance
            else if (damageInfo->GetSpellProto()->GetSpellIconID() == 186)
            {
                if (pVictim->isFrozen() || IsIgnoreUnitState(damageInfo->GetSpellProto(), IGNORE_UNIT_TARGET_NON_FROZEN))
                {
...
void Unit::SpellDamageBonusTaken(DamageInfo* damageInfo, uint32 stack)
{
    ...
    if (damageInfo->GetSchoolMask() & SPELL_SCHOOL_MASK_MAGIC)
    {
        // Ebon Plague
        if (Aura const* ebonPlague = GetAura(65142, EFFECT_INDEX_0))
            ...
            if (procSpell)
            {
                if (spellProcEvent)
                {
                    if (spellProcEvent->spellFamilyMask[i])
                    {
                        ...
                        // Backdraft hack
                        if (triggeredByAura->GetId() != 54274 && triggeredByAura->GetId() != 54276 && triggeredByAura->GetId() != 54277)
                        {
                            if (IsCastEndProcModifierAura(triggeredByAura->GetSpellProto(), SpellEffectIndex(i), procSpell))
                            {
...
void Unit::RemoveAurasByMechanicMask(uint32 mechMask, bool positiveOnly, uint32 count)
{
    ...
    for(Unit::SpellAuraHolderMap::iterator iter = holders.begin(); iter != holders.end() && count && uCount < count;)
    {
