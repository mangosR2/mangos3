...
void TargetedMovementGeneratorMedium<T, D>::_setTargetLocation(T& owner, bool updateDestination)
{
    ...
    if (updateDestination || !m_path)
    {
        ...
        else
        {
            ...
            m_target->GetNearPoint(&owner, x, y, z, owner.GetObjectBoundingRadius(), GetDynamicTargetDistance(owner, false), absAngle);
...
bool TargetedMovementGeneratorMedium<T, D>::RequiresNewPosition(T& owner, float x, float y, float z) const
{
    if ((owner.GetTypeId() == TYPEID_UNIT && ((Creature*)&owner)->CanFly()) || owner.IsLevitating())
        return !m_target->IsWithinDist3d(x, y, z, GetDynamicTargetDistance(owner, true));
    else
        return !m_target->IsWithinDist2d(x, y, GetDynamicTargetDistance(owner, true));
}
...
float ChaseMovementGenerator<T>::GetDynamicTargetDistance(T& owner, bool forRangeCheck) const
{
    if (forRangeCheck)
        ...
        return m_offset + CHASE_DEFAULT_RANGE_FACTOR * m_target->GetCombatReach(&owner);
}
