...
SpellAuraProcResult Unit::HandleDummyAuraProc(Unit *pVictim, DamageInfo* damageInfo, Aura const* triggeredByAura, SpellEntry const * procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    switch (dummySpell->GetSpellFamilyName())
    {
        ...
        {
            // Permafrost
            if (dummySpell->GetSpellIconID() == 143)
            {
            ...
            // Piercing Chill
            else if (dummySpell->GetSpellIconID() == 4625)
            {
...
*/
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            else if (dummySpell->GetSpellIconID() == 3186)
            {
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            if (dummySpell->GetSpellIconID() == 536)
            {
                ...
                // ToDo: Make it over DamageInfo struct
            ...
            // Wild Quiver
            else if (dummySpell->GetSpellIconID() == 2935)
            {
                if (triggeredByAura->GetEffIndex() != EFFECT_INDEX_0)
                    return SPELL_AURA_PROC_FAILED;
            ...
            else if (dummySpell->Id == 82661)               // Aspect of the Fox                                   
            {
                basepoints[0] = triggerAmount;
                ...
                {
                    if ((*itr)->GetSpellProto()->GetSpellIconID() == 5080 && (*itr)->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_HUNTER)
                    {
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            // Elemental Overload
            if (dummySpell->GetSpellIconID() == 2018 && effIndex == EFFECT_INDEX_0)            // only this spell have SpellFamily Shaman SpellIconID == 2018 and dummy aura
            {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            {
                basepoints[0] = int32((damage + damageInfo->absorb) * triggerAmount / 100);
...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(Unit *pVictim, DamageInfo* damageInfo, Aura const* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->GetSpellFamilyName())
    {
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            // Efflorescense
            else if (auraSpellInfo->GetSpellIconID() == 2886)
            {
        ...
        {
            // Shadow Infusion
            if (auraSpellInfo->GetSpellIconID() == 152)
            {
                Unit* pet = GetPet();
            ...
            else if (auraSpellInfo->GetSpellIconID() == 1762)
            {
...
SpellAuraProcResult Unit::HandleSpellAuraOverrideActionbarSpellsProc(Unit* /*pVictim*/, DamageInfo* /*damageInfo*/, Aura const* triggeredByAura, SpellEntry const *procSpell, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 /*cooldown*/)
{
    SpellEntry const* spellProto = triggeredByAura->GetSpellProto();
    int32 triggerAmount = triggeredByAura->GetModifier()->m_amount;
